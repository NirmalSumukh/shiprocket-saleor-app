...
/**
 * Saleor webhook for PRODUCT_CREATED events.
 * Uses the official SDK to verify JWS signatures via JWKS.
 */
export const productCreatedWebhook = new SaleorAsyncWebhook({
  name: 'Product Created - Sync to ShipRocket',
  webhookPath: 'api/shiprocket/webhooks/saleor-product-created',
...
export default productCreatedWebhook.createHandler(async (req, res, ctx) => {
  ...
  if (!product || !product.id) {
    logger.warn('Invalid product webhook payload (created)', payload);
    return res.status(400).json({ error: 'Invalid payload' });
  }

  logger.info('Received product create webhook from Saleor', {
  ...
  try {
    ...
    if (!result.success) {
      logger.error('Failed to sync created product to ShipRocket', {
        productId: product.id,
        error: result.error,
      });
    ...
    logger.info('Successfully synced created product to ShipRocket', { productId: product.id });
